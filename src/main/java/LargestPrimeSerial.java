...
/**
 * Congratulations, you’re done! The parallelized program should now be runnable.
 *
 * Run LargestPrimeForkJoin.java and check the largest prime it finds within its
 * allotted 5 seconds of runtime. Compare it to the serial version of the program.
 *
 * Are they equivalent? Is one faster than the other? Why might overhead cause the parallel program to be slower? Or, if it’s faster, what advantages would
 * parallelizing the program provide to make it faster? Was this tradeoff worth it?
 *
 * Why is this important? Large prime number primality tests are extremely
 * relevant for programmers, especially considering the security side of the
 * field (as a lot of encryption algorithms heavily rely on the computational
 * difficulty of large primes).
 *
 *
 * If you’d like to see what happens if you increase the time allotted for the program to run, try it out! In any case, congratulations on finishing the lesson!
